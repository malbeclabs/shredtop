...
pub struct FanInSource {
    ...
    /// Optional wire-level source IP filter. When set, the race tracker counts
    /// per-source shreds arriving from this specific IP (the validator's dz_ip).
    pub ip_filter: Option<u32>,
}

impl FanInSource {
    pub fn new() -> Self {
        Self { sources: Vec::new(), filter_programs: Vec::new(), leader_cache: None, ip_filter: None }
    }
    ...
    pub fn start(
        ...
        let race_tracker = ShredRaceTracker::new(shred_source_count, self.leader_cache, self.ip_filter);
